...
const formSchema = z.object({
  ...
  provider: z.string().min(1,'Informe o fornecedor').max(20, "Máximo de 20 caracteres"),
...
export function NewActiveForm() {
  ...
  return (
    <Form {...form}>
      <form className="flex flex-col gap-5" onSubmit={form.handleSubmit(handleSubmitForm)}>
        <div className="grid grid-cols-2 gap-2 ">
          <InputDialog nameInput="category" label="Categoria" controlInput={form.control} type="text"/>
          <InputDialog nameInput="description" label="Descrição" controlInput={form.control} type="text"/>
          <InputDialog nameInput="value_buy" label="Valor compra" controlInput={form.control} type="number"/>
          <InputDialog nameInput="provider" label="Fornecedor" controlInput={form.control} type="text"/>
          <InputDialog nameInput="nf" label="Nota Fiscal" placeholder="Opcional" controlInput={form.control} type="text"/>
          <InputDialog nameInput="rate" label="Taxa de depreciação" controlInput={form.control} type="number"/>
          <InputDialog nameInput="locale" label="Localização" controlInput={form.control} type="text"/>
          <InputDialog nameInput="date_buy" label="Data aquisição" controlInput={form.control} type="date"/>
          <InputDialog nameInput="life_util" label="Data final" controlInput={form.control} type="date"/>
        </div>
